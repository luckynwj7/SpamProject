...
with open('spam.pickle', 'wb') as file:
    pickle.dump(sFilter, file)

"""
print("스팸메일 아닌 예시")
hamExam = open("hamExam.txt", 'r').read()
pre = sFilter.predict(hamExam)
print("결과 =", pre)

print("")

print("스팸메일인 예시")
spamExam = open("spamExam.txt", 'r').read()
pre = sFilter.predict(spamExam)
print("결과 =", pre)
